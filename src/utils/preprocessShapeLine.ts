...
export async function preprocessShapeLines(
  buffer: ArrayBuffer
): Promise<ArrayBuffer> {
...
function transformDocumentXml(xml: string): string {
  ...
  for (const p of paragraphs) {
    ...
    for (const shapeEl of shapes) removeShapeWrapper(p, shapeEl);
...
function findShapeLines(p: Element): Element[] {
  const out: Element[] = [];

  for (const d of Array.from(p.getElementsByTagNameNS(W_NS, "drawing"))) {
  ...
  for (const pict of Array.from(p.getElementsByTagNameNS(W_NS, "pict"))) {
...
function readColor(shapeEl: Element): string | undefined {
...
function readSizeEighths(shapeEl: Element): number | undefined {
...
function parseStrokeWeightPt(weight: string | null): number | null {
...
function removeShapeWrapper(p: Element, shapeEl: Element) {
  let node: Node = shapeEl;
  while (node.parentNode && node.parentNode !== p) {
    const parent = node.parentNode as Element;
    const isRun = parent.namespaceURI === W_NS && parent.localName === "r";
    const isAltContent =
      parent.namespaceURI === MC_NS && parent.localName === "AlternateContent";
    if (isRun || isAltContent) {
      node = parent;
      if (isRun) break;
      continue;
    }
